...
if __name__ == '__main__' or _in_freecad:  # Show detailed status only in FreeCAD
    ...
    print(f"\nIntegration Status: {'COMPLETE' if status['integration_complete'] else 'PARTIAL'}")
    print("="*50)
elif not _in_freecad:
    # Minimal status for standalone
    status = get_integration_status()
    print(f"StructureTools v2.0.0 - {'Ready' if status['integration_complete'] else 'Partial'}")